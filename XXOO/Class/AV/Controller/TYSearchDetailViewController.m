...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.dataArr&&self.dataArr.count>indexPath.row) {
        ...
        __block TYSearchDetailCollectionViewCell *blockCell = cell;
        TYWEAK_SELF;
...
//收藏请求
- (void)shouCangRequestData:(TYHomeItemModel *)model ContentCell:(TYSearchDetailCollectionViewCell *)cell {
    NSDictionary * dic = @{
                           @"id":[TYGlobal userId],
                           @"tid":model.ID,
                           @"type":@"1"
    ...
    [TYNetWorkTool postRequest:@"/userCollection/api/addCollection" parameters:dic successBlock:^(BOOL success, id  _Nonnull data, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        if (success&&data) {
            [cell.collectionBtn setImage:[UIImage imageNamed:@"shoucang_image"] forState:UIControlStateNormal];
            model.cstate = @"1";
...
//收藏请求
- (void)cancelShouCangRequestData:(TYHomeItemModel *)model ContentCell:(TYSearchDetailCollectionViewCell *)cell {
    NSDictionary * dic = @{
                           @"uid":[TYGlobal userId],
                           @"tid":model.ID
                           };
    ...
    [TYNetWorkTool postRequest:@"/userCollection/api/delete" parameters:dic successBlock:^(BOOL success, id  _Nonnull data, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        if (success&&data) {
            [cell.collectionBtn setImage:[UIImage imageNamed:@"home_add"] forState:UIControlStateNormal];
            model.cstate = @"0";
        }else {
            [MBProgressHUD promptMessage:msg inView:self.view];
        }
